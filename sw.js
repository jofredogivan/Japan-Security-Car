...
// 3. MOVIMENTAÇÃO E ASSINATURA (CORREÇÃO TOUCH CELULAR)
function setupMovimentacaoForm() {
    ...
    // Força o ajuste do canvas quando a aba é exibida
    ...
    // Eventos Touch (passive: false é a chave para funcionar no mobile)
...
async function loadVeiculosList() {
    ...
    dash.innerHTML = cad.innerHTML = vs.map(v => {
        ...
        return `<div class="card" style="border-left:5px solid ${troca ? 'red' : 'green'};">
            ...
            <p style="color:${troca ? 'red' : 'green'}; margin:0; font-size:12px;">${troca ? '🚨 REVISÃO NECESSÁRIA' : '✅ Óleo OK'}</p>
            <button onclick="window.delV('${v.placa}')" style="font-size:10px; margin-top:5px; background:#444; color:white; border:none; padding:3px 8px; border-radius:4px;">Excluir Viatura</button>
        </div>`;
    }).join('');
}
